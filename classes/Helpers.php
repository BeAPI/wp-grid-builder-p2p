 ...
 *
 * Class API
 * @package WP_Grid_Builder\P2P_Facet
 */
class Helpers {
	...
	public static function locate_template( string $tpl ) {
		...
		$path = apply_filters( 'beapi_helpers_locate_template_templates', [ 'views/' . WP_GRID_BUILDER_P2P_FACET_VIEWS_FOLDER_NAME . '/' . $tpl . '.php' ], $tpl, __NAMESPACE__ );
		...
		// Locate on the files
		if ( is_file( WP_GRID_BUILDER_P2P_FACET_DIR . 'views/' . $tpl . '.php' ) ) {// Use builtin template
			return ( WP_GRID_BUILDER_P2P_FACET_DIR . 'views/' . $tpl . '.php' );
		}
	 ...
	 * @return array
	 */
	public static function get_facets_by_source_name( string $source_name ): array {
		global $wpdb;

		// Query post facets.
		return $wpdb->get_results(
			$wpdb->prepare(
				"SELECT * FROM {$wpdb->prefix}wpgb_facets
				WHERE source = %s",
				sprintf( 'post_meta/%s', $source_name ),
			),
			ARRAY_A
		);
	}
